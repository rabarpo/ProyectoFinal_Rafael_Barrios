...
|-----|--------------|------------------------------|
| `administrador` | `procesos`, `proceso-nuevo` | académica, usuarios, configuración, importación Excel |
| `director` | `procesos`, `proceso-nuevo` | académica, usuarios, configuración, importación Excel |
| `comite` | `procesos`, `proceso-nuevo` | académica (`comite` sí tiene `@Roles('administrador','director','comite')` en los `GET` de `anios-escolares.controller.ts`, `aulas.controller.ts`, `grados.controller.ts`, `niveles.controller.ts`; usuarios/configuración/importación siguen excluyéndolo) |
...
el rol de la sesión — ni más ni menos.
...
- WHEN observa el item "Académica" del menú
