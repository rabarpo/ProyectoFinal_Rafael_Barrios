...
- [x] 6.2 API-level smoke on the running dev stack (docker compose, `seed-estudiante` + a manually seeded open `DerechoVoto`): `POST /auth/login` then `GET /votos/mis-derechos` returns `200` with the expected shape (`derecho_voto_id`, `en_calidad_de`, `ya_voto:false`, `proceso.{id,nombre,tipo,fecha_cierre_prevista}`, no election-revealing fields). Browser-level click-through (card visible, click navigates to `/votar/:derechoVotoId`) was NOT done — browser automation was declined for this session — but is covered by `MisVotacionesPage.spec.tsx`'s RTL assertions against this exact response shape.
- [x] 6.3 API-level smoke: `seed-docente` login → `GET /votos/mis-derechos` returns `200 []` (no `DerechoVoto` rows), confirming the menu-driven "no card" behavior has no data to render. Browser-level confirmation of `InicioPage`'s unchanged empty state was NOT done for the same reason as 6.2, but is covered by `InicioPage.spec.tsx`'s docente case.
