...
/**
 * administracion-academica, PR2/PR4 (design.md D0, tareas 1.1-1.2, 7.8, 13.10, 14.10). `imports:
 ...
 * desde PR4) conforme se registra en los PR siguientes (D6 de `auth-server-sessions`, nunca en
...
@Module({
  ...
  controllers: [AniosEscolaresController, NivelesController, GradosController],
  providers: [PrismaService, AniosEscolaresService, NivelesService, GradosService],
})
export class AcademicoModule implements NestModule {
  configure(consumer: MiddlewareConsumer): void {
    consumer
      .apply(cookieParser())
      .forRoutes(AniosEscolaresController, NivelesController, GradosController);
  }
}
